...
class MutableCountingInclusionPreferringLazyZigzagPairingHeap(MutableCountingPriorityQueue):
    """A heap that is mutable, counting, inclusion preferring, lazy, and zigzag pairing.
    ...
    def ensure_top_demoted(self):
        ...
        if self.top_item is None:
            return
        demoted = MutableCountingInclusionPreferringLazyZigzagPairingHeap(self.top_item, self.forest, self.length)
        self.top_item = None
    ...
    def add(self, item):
        ...
        heap = MutableCountingInclusionPreferringLazyZigzagPairingHeap(top_item=item)
    ...
    def peek(self):
        ...
        if self.is_empty():
            raise IndexError("MutableCountingInclusionPreferringLazyZigzagPairingHeap: peek when empty.")
    ...
    def pop(self):
        ...
        if self.is_empty():
            raise IndexError("MutableCountingInclusionPreferringLazyZigzagPairingHeap: pop when empty.")
        self.ensure_top_promoted()
        item = self.top_item
        self.top_item = None
        self.length -= 1
...
def mciplzph_sorted(source):
    """Return new list of items, sorted using the mciplzp heap."""
    return sorted_using_mutable_unstable_counting_heap(MutableCountingInclusionPreferringLazyZigzagPairingHeap, source)
